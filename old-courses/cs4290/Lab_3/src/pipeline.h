...
typedef struct Pipeline {
  ...
  //Exception Handling -  you are not required to use the following variables, but you may find those helpful
  int32_t remaining_exception_handler_cycles; //to keep track of remaining cost of currently "executing" exception handler
  std::queue<Inst_Info> recovery_window; //this does not correspond to a physical hardware component of the OoO processor
  uint64_t last_recovery_inst; //keep track of the last instruction from the recovery window
  int32_t exception_index; //keeping track of how many exceptions encountered so far
