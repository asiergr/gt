...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i in range(len(self.weights)):
            s = temp[i] * alpha * delta
            self.weights[i], totalModification = (
                self.weights[i] + s,
                totalModification + abs(s),
            )
...
class NeuralNet(object):
    ...
    def __init__(self, layerSize):  # default 3 layer, 1 percep per layer
        ...
        """
        self.layerSize = (
            layerSize  # Holds number of inputs and percepetrons in each layer
        )
        self.outputLayer = []
        ...
        self.layers = [self.hiddenLayers[h] for h in range(self.numHiddenLayers)] + [
            self.outputLayer
        ]
            ...
            for layerNum in range(self.numHiddenLayers - 1, -1, -1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    delta = (
                        sum(
                            [
                                nextLayer[i].weights[neuronNum + 1] * deltas[0][i]
...
def buildNeuralNet(
    ...
    while iteration < maxItr and weightMod > weightChangeThreshold:
        ...
        if iteration % 10 == 0:
            print(
                f"! on iteration {iteration}; training error {trainError} and weight change {weightMod}."
            )
        else:
            print(".")
