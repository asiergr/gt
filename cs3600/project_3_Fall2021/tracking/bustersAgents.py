...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        closest_ghost = min(
            ghost_pos, key=lambda x: self.distancer.getDistance(pacmanPosition, x)
        )
        action = min(
            legal,
            key=lambda x: self.distancer.getDistance(
                Actions.getSuccessor(pacmanPosition, x), closest_ghost
            ),
        )
        return action
