...
class CustomReLU(TorchFunc):
    ...
    def backward(self, y):
        ...
        deconvolution = torch.addcmul(torch.zeros_like(y), y, (y > 0).type_as(y))
        back = torch.addcmul(torch.zeros_like(deconvolution), deconvolution, (x > 0).type_as(x))
        return back
...
class GradCam:
    def guided_backprop(self, X_tensor, y_tensor, gc_model):
        ...
        ##############################################################################
        pred = gc_model(X_tensor)
        pred.backward(torch.ones_like(pred))
        out = torch.transpose(X_tensor.grad, 1, 3)
    ...
    def grad_cam(self, X_tensor, y_tensor, gc_model):
        ...
        pred = gc_model(X_tensor)
        pred.backward(torch.ones_like(pred))
        cam = self.activation_value * self.gradient_value
