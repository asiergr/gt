...
class StyleLoss(nn.Module):
    def gram_matrix(self, features, normalize=True):
        ...
        F_l = features.reshape(N, C, -1)
        G_l = torch.bmm(F_l, F_l.transpose(1, 2))

        if normalize: return G_l / (C * H * W)
    ...
    def forward(self, feats, style_layers, style_targets, style_weights):
        ...
        for l in range(len(style_layers)):
            G = self.gram_matrix(feats[style_layers[l]])
            loss += style_weights[l] * torch.sum(torch.square(G - style_targets[l]))
