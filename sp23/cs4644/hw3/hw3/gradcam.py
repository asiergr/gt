...
def guided_gradcam_captum():
    ...
    ##############################################################################
    guided_gc = GuidedGradCam(model, model.features[3])
    attr_gc = compute_attributions(guided_gc, X_tensor, target=y_tensor)
    plt = visualize_attr_maps("Guided GradCAM Captum", X, y, class_names, [attr_gc], ["GuidedGradCam"])

    guided_bp = GuidedBackprop(model)
    attr_bp = compute_attributions(guided_bp, X_tensor, target=y_tensor)
    plt = visualize_attr_maps("Guided Backprop Captum", X, y, class_names, [attr_bp], ["GuidedBackprop"])
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return plt
...
def layer_vis_captum():
    ...
    visualize_attr_maps("Layer GradCAM Captum", X, y, class_names, [layer_gc_attr_sum], ["LayerGradCam"])

    layer_conduct = LayerGradCam(model, layer)
    ...
    visualize_attr_maps("Layer Conductance Captum", X, y, class_names, [layer_conduct_attr_sum], ["LayerConductance"])
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return plt
